...
本文沿用带符号精确二面角 \(\phi\) 的定义，只把半边向量修改为：
...
注意 \(J_{S,a}^{(\sigma)}\) 和 \(J_{S,b}^{(\sigma)}\) 与 \(\sigma\) 无关。

### 2.4 新半边向量 \(N_\sigma=\phi S_\sigma\) 的雅可比
...
反向半边用同一组正向局部变量 \(e,a,b\) 表示时取 \(\sigma=-1\)：
...
这三式可以直接用于能量梯度计算，不需要显式构造 \(3\times3\) 矩阵。
...
正向半边 \(N_+\) 进入 \(\bar N(v)\)，反向半边 \(N_-\) 进入 \(\bar N(v_i)\)。

由于移动 \(v=p_1\) 时，在正向局部变量中只改变 \(e\)，所以单条邻接边对梯度的贡献为：
...
这里每条半边的 \(e,a,b,\phi,S_\pm,P_e,P_a,P_b,g_e\) 都使用该半边自己的局部几何量。

与旧公式相比，只有边方向项出现
\(\bar N(v)-\bar N(v_i)\)，两个面法向方向项对应的是
\(\bar N(v)+\bar N(v_i)\)。
...
正向半边 \(N_+\) 进入 \(\bar N(v_i)\)，反向半边 \(N_-\) 进入 \(\bar N(v_j)\)。
...
如果拓扑遍历可以统一选择半边方向，使待求顶点始终位于 \(p_2\)，则实现中只需要保留 \(E_{2,a}\) 分支。
...
对应三角形退化，\(n_a,n_b\)、角度梯度和雅可比均无法定义，应跳过该半边或先修复网格。
...
但是 \(J_e,J_a,J_b\) 中的 \(S_\sigma g^T\) 项一般仍然存在，因此不能因为 \(\phi=0\) 就直接把雅可比设为零。
...
且 `atan2` 的第一个参数接近零时，\(|\phi|\approx\pi\)。

此时 \(\phi\) 位于 \(+\pi/-\pi\) 分支附近，角度本身不连续。建议检测并跳过完全翻折状态，或者对 \(\phi\) 做跨迭代连续展开。
