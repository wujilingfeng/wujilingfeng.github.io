...
八叉树的节点和根节点的关系（或者路径）都可以用3向量2进制数表示，它的邻接关系也一目了然（通过在每个向量的x,y,z位置加减，就能得到该x,y,z方向的邻接元素）。
...
对于小数来说，它不仅满足进制的性质（低位满数向前进一），而且还满足其后加多少0都相等。

小数的进制形式是$1,\frac{1}{10},\frac{1}{10^2}...$,

八叉树体素的每个方向都是二叉树，每个xyz方向的位置是$\frac{1}{2}$进制的.,也就是八叉树节点的坐标值是1 ,(1/2)... (1/2^depth)
