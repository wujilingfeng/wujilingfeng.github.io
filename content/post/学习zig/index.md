...
结构体，联合体等容器的test函数默认不执行。但当文件容器的test函数里面使用了该容器，这样该容器里面的test函数会被执行。
...
还应该注意运算级的顺序问题，比如 ((@as(usize, 1) << (depth * BITS_PER_LAYER)) - 1)， r和 (@as(usize, 1) << (depth * BITS_PER_LAYER) - 1)运算完全不同，后者会先计算< (depth * BITS_PER_LAYER) - 1，因为位运算的优先级比较低。
